...
C言語とWin32 APIだけで作る、学習用の月間カレンダーGUIです。将来的にはカレンダーに予定を入力するアプリへ拡張しますが、v0.1では日付計算・自前描画・クリック選択に絞っています。カレンダー本体は常に7列×6行、合計42セルを先に描画し、その後に当月の日付だけをセルへ割り当てます。

## 構成

- `src/calendar.c` / `src/calendar.h`: うるう年、月の日数、曜日、前月・翌月、42セルへの日付割り当て
- `src/main.c`: Win32ウィンドウ、42セルのGDI描画、クリック処理
